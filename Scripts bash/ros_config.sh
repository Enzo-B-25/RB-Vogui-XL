...
if $ROBOT_ACKERMANN_JOINTS
then
   echo "ROBOT_ACKERMANN_JOINTS is $ROBOT_ACKERMANN_JOINTS, so the ackerman configuration located at $robot_params/ackerman_params.env will load"
   source $robot_params/ackermann_params.env
fi
