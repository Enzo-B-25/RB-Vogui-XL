...
if [[ -z "${ROBOT_RUN_INTERFACES}" ]]; then
  ROBOT_RUN_INTERFACES=true
fi
if [[ -z "${ROBOT_RUN_SAFETY}" ]]; then
  ROBOT_RUN_SAFETY=true
fi
...
then
  ...
  echo_success "Launching robot $ROBOT_BRINGUP_PACKAGE/$ROBOT_BRINGUP_LAUNCH..."
